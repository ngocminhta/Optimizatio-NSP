class LessOrEqualFunction:
	...
	def initPropagation(self):
		if self.__f__.getValue() <= self.__v__.getValue():
			self.__violations__ = 0
		else:
			self.__violations__ = self.__f__.getValue() - self.__v__.getValue()
	...
	def getSwapDelta(self,x,y):
		...
		new_violations = 0
		if nv > self.__v__.getValue():
			new_violations = nv - self.__v__.getValue()
